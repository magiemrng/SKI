    ...
    <section id="services" className="py-32 bg-gradient-to-br from-gray-50 via-white to-gray-50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            <Sparkles className="w-5 h-5 text-ski-accent" />
            <span className="text-ski-accent font-semibold">Our Expertise</span>
          </motion.div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, index) => (
            ...
            >
              {/* Main Card */}
              <div className="relative bg-white/80 backdrop-blur-xl p-8 rounded-3xl border border-gray-200/50 shadow-lg hover:shadow-2xl transition-all duration-500 h-full overflow-hidden">
                {/* Gradient Background on Hover */}
                <motion.div
                  className={`absolute inset-0 bg-gradient-to-br ${service.bgGradient} opacity-0 group-hover:opacity-100 transition-all duration-500 rounded-3xl`}
                ...
                >
                  <div className={`inline-flex items-center justify-center w-16 h-16 ${service.iconBg} rounded-2xl shadow-lg group-hover:shadow-xl transition-all duration-300 relative overflow-hidden`}>
                    {/* Icon Glow Effect */}
                ...
                <div className="relative z-10">
                  <h3 className="text-xl font-bold text-ski-black mb-4 group-hover:text-gray-800 transition-colors duration-300">
                    {service.title}
                  </h3>
                  ...
                  <ul className="space-y-2 mb-6">
                    {service.features.map((feature, idx) => (
                      <motion.li 
                        key={idx} 
                        className="text-sm text-gray-500 flex items-center group-hover:text-gray-600 transition-colors duration-300"
                        initial={{ opacity: 0, x: -10 }}
                        animate={inView ? { opacity: 1, x: 0 } : {}}
                        transition={{ duration: 0.4, delay: 0.6 + index * 0.1 + idx * 0.05 }}
                      >
                        <motion.div 
                          className="w-1.5 h-1.5 bg-gradient-to-r from-ski-accent to-orange-500 rounded-full mr-3 flex-shrink-0"
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-full bg-gradient-to-r from-gray-800 to-gray-900 text-white py-3 px-6 rounded-xl font-medium flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 hover:shadow-lg"
                  >
                    Learn More
              ...
              <motion.div
                ...
                transition={{ duration: 0.3 }}
                className="absolute -top-3 -right-3 bg-gradient-to-r from-ski-accent to-orange-500 text-white px-3 py-1 rounded-full text-xs font-semibold shadow-lg opacity-0 group-hover:opacity-100"
              >
                Popular
              </motion.div>
            </motion.div>
        ...
        >
          <div className="bg-gradient-to-r from-ski-black to-gray-800 rounded-3xl p-12 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-ski-accent to-orange-500 text-white px-8 py-4 rounded-xl font-semibold flex items-center gap-2 mx-auto hover:shadow-xl transition-all duration-300"
              >
                Get Started Today
